...
**Run the scanner instantly, no install, no Python** (once this is pushed to your GitHub repo — replace `<you>`):
```bash
npx github:<you>/vibecoded-app-security /path/to/your/project
npx github:<you>/vibecoded-app-security /path/to/your/project --json
```
...
```bash
git clone https://github.com/<you>/vibecoded-app-security.git ~/.claude/skills/vibecoded-app-security
```
...
Found a vulnerability pattern that keeps showing up in vibecoded apps and isn't covered here? Open a PR against `references/checklist.md` or `references/stack-notes.md`, or add a rule to `scripts/quick_scan.py`.
